...
loopback.checkModelRegistrySupport = function() {
  ...
  if (hasOwnerAwareMethods && !ModelRegistry._aliasesCreated) {
    ...
    ModelRegistry.getModelsForOwner = function(owner, ownerType) {
      if (arguments.length === 2) {
        // Use explicit API for App ownership, simplified API for DataSource ownership
        if (ownerType === 'app') {
          return ModelRegistry.getModelsForOwnerWithType(owner, ownerType);
        } else {
          // DataSource ownership: use simplified API (auto-detect owner type)
    ...
    ModelRegistry.getModelForOwner = function(modelName, owner, ownerType) {
      if (arguments.length === 3) {
        // Use explicit API for App ownership, simplified API for DataSource ownership
        if (ownerType === 'app') {
          return ModelRegistry.getModelForOwnerWithType(modelName, owner, ownerType);
        } else {
          // DataSource ownership: use simplified API (auto-detect owner type)
        ...
        // Use simplified API (auto-detect owner type)
        return originalGetModelForOwner.call(this, modelName, owner);
      }
    };
  ...
  return {
    ...
    methods: hasOwnerAwareMethods ? [
      'ModelRegistry.getModelsForOwner(owner, ownerType)',
      'ModelRegistry.getModelNamesForOwner(owner, ownerType)',
      'ModelRegistry.hasModelForOwner(modelName, owner, ownerType)',
      'ModelRegistry.getModelForOwner(modelName, owner, ownerType)'
    ] : []
  };
};
