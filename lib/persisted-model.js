...
module.exports = function(registry) {
  ...
  PersistedModel.replicate = async function(since, targetModel, options = {}) {
    const debug = require('debug')('loopback:persisted-model:replicate')
    ...
    try {
      ...
      if (!this.getChangeModel()) {
        throw new Error('Change tracking not enabled')
      }

      // Ensure models are attached to datasources
      if (!this.dataSource) {
        throw new Error('Source model not attached to a datasource')
      }
      
      if (!targetModel.dataSource) {
        throw new Error('Target model not attached to a datasource') 
      }
