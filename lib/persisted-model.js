    ...
    try {
      ...
      try {
        ...
        if (updates.length > 0) {
          ...
          bulkUpdateResult = await targetModel.bulkUpdate(updates, bulkUpdateOptions)
        }
        ...
        if (bulkUpdateResult.conflicts && bulkUpdateResult.conflicts.length > 0) {
          ...
          const bulkUpdateConflicts = bulkUpdateResult.conflicts.map(function(conflict) {
            ...
            // Create a new Conflict instance
            return new Change.Conflict(
              conflict.modelId, sourceModel, targetModel, conflictData
            )
          })
          ...
          conflicts.push(...bulkUpdateConflicts)
        }
            ...
            if (existing) {
              ...
              if (update.change) {
                ...
                if (rev !== update.change.prev) {
                  ...
                  } else if (options.conflictResolution) {
