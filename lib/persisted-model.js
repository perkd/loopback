...
module.exports = function(registry) {
  ...
  PersistedModel.replicate = async function(targetModel, since = -1, options = {}) {
    ...
    // Keep replicating while there are changes
    while (true) {
      // Check if there are more changes to replicate
      const hasChanges = await new Promise((resolve, reject) => {
        sourceModel.hasChangesSince(since.source, options.filter, (err, result) => {
          if (err) reject(err)
          else resolve(result)
        })
      })
      ...
      try {
        ...
        // If there are conflicts, stop replication
  ...
  async function getDiffFromTarget(targetModel, since, sourceChanges, chunkSize) {
    const diff = await uploadInChunks(
      sourceChanges,
      chunkSize,
      async (chunk) => targetModel.diff(since, chunk)
    )

    if (debug.enabled) {
      if (diff.conflicts?.length) {
        debug('\tdiff conflicts')
        diff.conflicts.forEach(d => debug('\t\t%j', d))
      }
      if (diff.deltas?.length) {
        debug('\tdiff deltas')
        diff.deltas.forEach(it => debug('\t\t%j', it))
      }
    }
