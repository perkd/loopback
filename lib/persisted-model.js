...
module.exports = function(registry) {
  ...
  PersistedModel.replicate = async function(since, targetModel, options = {}) {
    ...
    if (!targetModel) {
      const err = new Error('Target model is required')
      err.statusCode = 400
      throw err
    }

    // Handle REST models
    const isRestModel = targetModel.createChangeStream && !targetModel.changes
    if (isRestModel) {
      try {
        const result = await targetModel.replicate({
          since: since,
          source: this,
        ...
        return {
          ...
          checkpoint: result.checkpoint || await this.currentCheckpoint()
        }
      } catch (err) {
    ...
    // Validate change tracking for local models
    const Change = this.getChangeModel()
    const TargetChange = targetModel.getChangeModel && targetModel.getChangeModel()
    if (!Change || !TargetChange) {
      const err = new Error('Change tracking must be enabled')
      err.statusCode = 400
      throw err
    }

    try {
      // Get changes from source and target
      const sourceChanges = await this.changes(since, options.filter)
      const targetChanges = await targetModel.changes(since, options.filter)

      // Get the diff
      const diff = await this.diff(since, targetChanges)
      ...
      debug('Error during replication:', err)
      if (!err.statusCode) err.statusCode = 500
      throw err
    }
  }
              ...
              switch (update.type) {
                case Change.UPDATE:
                  if (current) {
                    ...
                    if (rev !== update.change.prev) {
                      debug('Detected non-rectified change of %s %j', this.modelName, id)
                    ...
                    debug('Instance not found for updating %s %j', this.modelName, id)
                    conflicts.push(update.change)
                    continue
                  }
                  break
                ...
                case Change.CREATE:
                  try {
                    ...
                    if (existing) {
                      debug('Detected non-rectified new instance of %s %j', this.modelName, id)
                      conflicts.push(update.change)
                      continue
                    }
                    throw createErr
