...
module.exports = function(registry) {
  ...
  PersistedModel.replicate = async function(since, targetModel, options = {}) {
    // Handle since parameter format
    ...
    const targetId = await targetModel.getSourceId()

    // Get changes since the given checkpoint
    const sourceChanges = await this.changes(since.source, options.filter)
    const targetChanges = await targetModel.changes(since.target, options.filter) 

    debug('replicating %s since %s from %s to %s',
      this.modelName,
      since,
      sourceId,
      targetId)
    ...
    debug('detected %d change(s) and %d conflict(s)',
      diff.deltas.length,
      diff.conflicts.length)

    // Create updates from deltas
    const updates = await targetModel.createUpdates(diff.deltas)

    // Apply updates
    const result = await targetModel.bulkUpdate(updates)

    // Get current checkpoints
    const sourceCheckpoint = await this.currentCheckpoint()
    const targetCheckpoint = await targetModel.currentCheckpoint()
  ...
  async function tryReplicate(sourceModel, targetModel, since, options) {
    ...
    if (!Change.Conflict) {
      ...
      Change.Conflict.prototype.resolveUsingSource = async function() {
        ...
        if (!source) {
          debug('resolveUsingSource: Source is null for modelId ' + this.modelId)
          // Instead of deleteById, check if target record exists and destroy it
          const targetRecord = await this.TargetModel.findById(this.modelId)
          if (targetRecord) {
            debug('resolveUsingSource: Found target record, calling destroy()')
            await targetRecord.destroy()
          } else {
            debug('resolveUsingSource: Target record already null')
          }
        } else {
          const data = (source.toObject ? source.toObject() : {}) || {}
          const inst = new this.TargetModel(data, { persisted: true })
          debug('resolveUsingSource: Saving updated instance for modelId ' + this.modelId)
          await inst.save()
        }
        await this.resolve()
      }
      ...
      Change.Conflict.prototype.resolveUsingTarget = async function() {
        ...
        const [source, target] = await this.models()
        if (!target) {
          debug('resolveUsingTarget: Target is null for modelId ' + this.modelId)
          const sourceRecord = await this.SourceModel.findById(this.modelId)
          if (sourceRecord) {
            debug('resolveUsingTarget: Found source record, calling destroy()')
            await sourceRecord.destroy()
          } else {
            debug('resolveUsingTarget: Source record already null')
          }
        } else {
          const data = (target.toObject ? target.toObject() : {}) || {}
          const inst = new this.SourceModel(data, { persisted: true })
          debug('resolveUsingTarget: Saving updated instance for modelId ' + this.modelId)
          await inst.save()
        }
        await this.resolve()
      }
    ...
    const conflicts = diff.conflicts.filter(change => change).map(change => new Change.Conflict(change.modelId, sourceModel, targetModel))
    if (conflicts.length) {
    ...
    return {
      conflicts,
      checkpoints: (targetModel === sourceModel) ? { source: newSourceCp, target: undefined } : { source: newSourceCp, target: newTargetCp },
      updates
    }
  }
  ...
  async function createSourceUpdates(sourceModel, deltas, replicationChunkSize) {
    ...
    const Change = sourceModel.getChangeModel()
    debug('createSourceUpdates: sourceModel ' + sourceModel.modelName + ' with ' + deltas.length + ' deltas')
    const updates = []
  ...
  PersistedModel.getSourceId = function() {
    const dataSource = this.dataSource
    if (!dataSource) {
      return this.once('dataSourceAttached', this.getSourceId.bind(this))
    }
    assert(
      dataSource.connector.name,
      'Model.getSourceId: cannot get id without dataSource.connector.name'
    )
    return [dataSource.connector.name, this.modelName.toLowerCase()].join('-')
  }
  ...
  PersistedModel.enableChangeTracking = function() {
    ...
    function rectifyOnSave(ctx, next) {
      // Use hookState to track across the entire operation chain
      ctx.hookState = ctx.hookState || {}
      if (ctx.hookState.__rectifySave) return next()
      ctx.hookState.__rectifySave = true

      const instance = ctx.instance || ctx.currentInstance
      const id = instance ? instance.getId() :
        getIdFromWhereByModelId(Model, ctx.where)

      if (debug.enabled) {
        debug('rectifyOnSave %s -> ' + (id ? 'id %j' : '%s'),
          Model.modelName, id ? id : 'ALL')
        debug('context instance:%j currentInstance:%j where:%j data %j',
          ctx.instance, ctx.currentInstance, ctx.where, ctx.data)
      }

      if (id != null) {
        return Model.rectifyChange(id)
          .then(() => next())
          .catch(err => {
            Model.handleChangeError(err, 'after save')
            next()
          })
      } else {
        return Model.rectifyAllChanges()
          .then(() => next())
          .catch(err => {
            Model.handleChangeError(err, 'after save')
            next()
    ...
    function rectifyOnDelete(ctx, next) {
      ctx.hookState = ctx.hookState || {}
      if (ctx.hookState.__rectifyDelete) return next()
      ctx.hookState.__rectifyDelete = true
      ...
      if (id != null) {
        return Model.rectifyChange(id)
          .then(() => next())
          .catch(err => {
            Model.handleChangeError(err, 'after delete')
            next()
          })
      } else {
        return Model.rectifyAllChanges()
  ...
  PersistedModel._defineChangeModel = function() {
    ...
    // Follow original implementation more closely
          ...
          const tasks = chunk.map(async update => {
            try {
              ...
              switch (update.type) {
                  ...
                  if (current) {
                    ...
                    if (rev !== update.change.prev) {
                      debug('Detected non-rectified change of %s %j',
                        this.modelName, id)
                      debug('\tExpected revision: %s', update.change.rev)
                    ...
                    debug('Instance not found for updating %s %j',
                      this.modelName, id)
                    conflicts.push(update.change)
                    return
                  }
                ...
                case Change.DELETE:
                  if (!current) {
                    // The instance was already deleted or not created
                    return
                  }
                  const rev = Change.revisionForInst(current)
                  if (rev !== update.change.prev) {
    ...
    const tasks = deltas.map(async changeData => {
      ...
      if (type === Change.CREATE || type === Change.UPDATE) {
        try {
          const inst = await Model.findById(change.modelId)
          if (!inst) {
            debug('Missing data for change: %s', change.modelId)
            return // Skip this update
          }
          update.data = inst.toObject ? inst.toObject() : inst
          updates.push(update)
        } catch (err) {
          debug('Error getting data for change: %s', err)
          return // Skip this update
        }
      } else if (type === Change.DELETE) {
        updates.push(update)
