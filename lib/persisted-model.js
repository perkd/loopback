...
module.exports = function(registry) {
  ...
  async function tryReplicate(sourceModel, targetModel, since, options) {
    ...
    const conflicts = diff.conflicts.map(change => new Change.Conflict(change.modelId, sourceModel, targetModel))
    if (conflicts.length) {
    ...
    return {
      conflicts,
      checkpoints: targetModel ? { source: newSourceCp, target: undefined } : { source: newSourceCp, target: newTargetCp },
      updates
    }
  }
  ...
  PersistedModel.enableChangeTracking = function() {
    ...
    function rectifyOnSave(ctx, next) {
      ...
      let id = instance ? instance.getId() : (ctx.where ? ctx.where.id : undefined)
      if (id) {
    ...
    function rectifyOnDelete(ctx, next) {
      ...
      let id = ctx.where ? ctx.where.id : undefined
      if (id) {
        Model.rectifyChange(id)
          .then(() => next())
          .catch(err => next(err))
