...
module.exports = function(registry) {
  ...
  PersistedModel.replicate = async function(since = -1, targetModel, options = {}) {
    ...
    while (true) {
      ...
      const { conflicts, updates, checkpoints } = result
  ...
  PersistedModel.getChangeModel = function() {
    ...
    debug('getChangeModel: for model ' + this.modelName + ', changeModel is ' + (changeModel ? 'defined' : 'undefined'))
  ...
  PersistedModel.enableChangeTracking = function() {
    ...
    function rectifyOnSave(ctx, next) {
      ...
      if (instance && instance.__rectifyCalled) return next()
      if (instance) instance.__rectifyCalled = true
      let id = instance ? instance.getId() : (ctx.where ? ctx.where[idName] : (ctx.data ? ctx.data[idName] : undefined))
      if (id) {
        Model.rectifyChange(id)
          .then(() => next())
          .catch(err => next(err))
      } else {
        Model.rectifyAllChanges()
          .then(() => next())
          .catch(err => next(err))
      }
    }
    ...
    function rectifyOnDelete(ctx, next) {
      const idName = Model.getIdName()
      if (ctx.where && ctx.where.__rectifyCalled) return next()
      if (ctx.where) ctx.where.__rectifyCalled = true
      let id = (ctx.where ? ctx.where[idName] : undefined) || (ctx.data ? ctx.data[idName] : undefined)
      if (id) {
        Model.rectifyChange(id)
          .then(() => next())
          .catch(err => next(err))
                ...
                case Change.UPDATE:
                  if (current) {
                    ...
                    if (rev !== update.change.prev) {
                      ...
                      // {{ Comment out rectifyModelChanges to prevent auto-resolution in tests }}
                    ...
                    // Treat missing instance as a conflict
                    conflicts.push(update.change)
                    // {{ Comment out rectifyModelChanges to prevent auto-resolution in tests }}
                    // await Change.rectifyModelChanges(this.modelName, [id])
                    return
