...
module.exports = function(registry) {
      ...
      const result = await uploadInChunks(
        ...
        async (chunk) => {
          try {
            ...
            const diffResult = await targetModel.diff(since, chunk)
  ...
  PersistedModel.bulkUpdate = async function(updates, options = {}) {
    ...
    debug('\tbulkUpdate: starting with %d updates for model %s', 
      updates ? updates.length : 0, this.modelName)
    
    if (!updates || updates.length === 0) {
      debug('\tbulkUpdate: no updates to process')
      return { results, conflicts: null }
    }
    
    try {
      // Build an efficient lookup of existing model instances
      const idName = this.getIdName()
      const ids = updates.map(update => update.id || (update.data && update.data.id)).filter(Boolean)
      const existingLookup = {}
      
      if (ids.length > 0) {
        debug('\tbulkUpdate: fetching existing instances by ids, %d ids', ids.length)
        const where = {}
        where[idName] = { inq: ids }
        const existing = await this.find({ where })
        
        existing.forEach(inst => {
          if (inst && inst[idName] != null) {
            existingLookup[inst[idName]] = inst
          }
        })
        debug('\tbulkUpdate: found %d existing instances', existing.length)
      }
      
      // Process updates sequentially to maintain order
      for (const update of updates) {
        if (!update || (!update.data && update.type !== 'delete')) {
          debug('\tbulkUpdate: skipping invalid update')
          continue
        }
        
        const id = update.id || (update.data && update.data.id)
        
        if (!id) {
          debug('\tbulkUpdate: skipping update with no id')
          continue
        }
        ...
        try {
          ...
          switch (update.type) {
            case 'delete':
              debug('\tbulkUpdate: deleting instance %s', id)
            ...
            case 'create':
            case 'update':
              const existing = existingLookup[id]
              
              if (existing) {
                debug('\tbulkUpdate: updating existing instance %s', id)
                // Use instance update to trigger observers and handle options
                result = await existing.updateAttributes(update.data, options)
              } else {
                debug('\tbulkUpdate: creating new instance %s', id)
                result = await this.create(update.data, options)
              }
              break
              
            default:
              debug('\tbulkUpdate: unknown update type: %s', update.type)
              continue
          }
          
          results.push(result)
          debug('\tbulkUpdate: successfully processed update for id %s', id)
          
        } catch (error) {
          debug('\tbulkUpdate: error processing update %s - %s', id, error.message)
          ...
          if (error && error.statusCode === 409) {
            ...
            conflicts.push({
              modelId: id,
              modelName: this.modelName,
              conflict: error.details || error.message
            })
          } else {
            // For non-conflict errors, still continue with other updates
            debug('\tbulkUpdate: non-conflict error: %s', error.stack || error)
          }
        }
      }
      
      debug('\tbulkUpdate: completed processing %d updates, %d results, %d conflicts', 
        updates.length, results.length, conflicts.length)
      ...
      debug('\tbulkUpdate: unexpected error: %s', err.stack || err)
      throw err
    }
  }
