...
module.exports = function(registry) {
  ...
  async function getDiffFromTarget(targetModel, since, sourceChanges, chunkSize) {
    ...
    // Ensure Change model is defined
    ...
    try {
      ...
      const diff = await uploadInChunks(
        ...
        async (chunk) => {
          try {
            const result = await targetModel.diff(since, chunk)
      ...
      // Ensure diff has expected properties before returning
      if (!diff) {
        debug('\tgetDiffFromTarget: diff is undefined, returning empty result')
        return { deltas: [], conflicts: [] }
      }
  ...
  PersistedModel.enableChangeTracking = function() {
    ...
    async function rectifyOnSave(ctx) {
      ...
      try {
        // Always use rectifyChange when we have an ID
        if (id != null) {
          // Create change record first
          ...
          debug('calling rectifyChange with id: %s', id)
          await ctx.Model.rectifyChange(id)
        } else {
          // Handle bulk operations like updateAll
          debug('calling findAffectedInstancesAndRectify (no id available)')
          await findAffectedInstancesAndRectify(ctx)
        }
      } catch (err) {
    ...
    // New helper function to handle bulk operations
    async function findAffectedInstancesAndRectify(ctx) {
      const Model = ctx.Model
      const Change = Model.getChangeModel()
      
      // Find all instances affected by the bulk operation
      const where = ctx.where || {}
      debug('Finding affected instances with where filter: %j', where)
      
      try {
        // Find all affected instances
        const instances = await Model.find({ where })
        debug('Found %d affected instances', instances.length)
        
        // Create change records for each instance
        for (const inst of instances) {
          const id = inst.getId()
          debug('Creating change record for instance %s', id)
        ...
        debug('Error finding affected instances: %s', err.message)
        // Fall back to rectifyAllChanges if we can't find affected instances
    ...
    async function rectifyOnDelete(ctx, next) {
      ...
      if (id != null) {
        // Handle single deletion - original behavior
        ctx.Model.rectifyChange(id, reportErrorAndNext)
      } else {
        // For bulk deletions, we need to handle differently
        // First check if we're dealing with a destroyAll operation with empty where clause
        const isEmptyWhere = !ctx.where || Object.keys(ctx.where).length === 0
        
        if (isEmptyWhere) {
          // This is a destroyAll() with empty where - we need to get all change records
          // and mark them as deleted since all instances are being deleted
          const Change = ctx.Model.getChangeModel()
          const changes = await Change.find({
            where: { modelName: ctx.Model.modelName }
          })
          
          debug('Found %d change records to mark as deleted', changes.length)
          
          // Mark each change as deleted
          ...
          // This is a targeted deleteAll with a where clause
          // We need to find the affected instances before they're deleted
          try {
            // Get the affected instances before they're deleted
            // This is tricky because the delete has already happened at this point
            // We need to rely on the change records
            const Change = ctx.Model.getChangeModel()
            const changes = await Change.find({
              where: { 
                modelName: ctx.Model.modelName,
                // We can't filter by the where clause directly since it's on the model, not the change
              }
            })
            
            debug('Found %d change records to process for deletion', changes.length)
            
            // Mark each change as deleted
            for (const change of changes) {
              // We would need to check if this instance matches the where clause
              // But that's difficult since the instance is already deleted
              // For now, mark all as deleted and let the next replication sort it out
              change.type = Change.DELETE
              await change.save()
            }
