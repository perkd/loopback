...
module.exports = function(registry) {
  ...
  PersistedModel.create = function(data, options, callback) {
    ...
    return new Promise((resolve, reject) => {
      this.base.create(data, options, (err, result) => {
        if (err) {
   ...
   */
  PersistedModel.checkpoint = function() {
   ...
   */
  PersistedModel.currentCheckpoint = function() {
    const Checkpoint = this.getChangeModel().getCheckpointModel();
  ...
  PersistedModel.replicate = function(since, targetModel, options, callback) {
    ...
    if (!callback) {
      return new Promise((resolve, reject) => {
        this.replicate(since, targetModel, options, (err, conflicts, checkpoints, updates) => {
          if (err) reject(err)
          else resolve({conflicts, checkpoints, updates})
        })
      })
    }
  ...
  PersistedModel.createChangeStream = function(options, cb) {
    ...
    if (!cb) {
      return new Promise((resolve, reject) => {
        this.createChangeStream(options, (err, result) => {
          if (err) reject(err)
          else resolve(result)
        })
      })
