...
module.exports = function(registry) {
  ...
  PersistedModel.replicate = async function(since, targetModel, options = {}) {
    // First ensure change tracking is enabled
    if (!this.changes || !targetModel.changes) {
      this.enableChangeTracking()
      targetModel.enableChangeTracking()
    }

    // Handle since parameter format first
    ...
    const diff = await this.getChangeModel().diff(
      this.modelName,
      since,
      sourceChanges,
      targetChanges
    ...
    const updates = await this.createUpdates(diff.deltas)
    ...
    return {
      checkpoints: {
        source: await this.currentCheckpoint(),
        target: await targetModel.currentCheckpoint()
      },
  ...
  // Add helper method like original
  PersistedModel.createUpdate = async function(delta) {
    const Change = this.getChangeModel()
    ...
    if (type === Change.CREATE || type === Change.UPDATE) {
      const inst = await this.findById(change.modelId)
      if (inst) {
  ...
  PersistedModel.getSourceId = function() {
    ...
    // Original returns null synchronously - don't try to be clever with async
    ...
    return dataSource.connector.name + '-' + this.modelName.toLowerCase()
  }
