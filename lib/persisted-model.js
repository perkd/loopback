...
module.exports = function(registry) {
  ...
  async function getDiffFromTarget(targetModel, since, sourceChanges, chunkSize) {
    ...
    try {
      ...
      // Always fetch changes from the target model first to ensure proper spying in tests
      // This is important even if there are no source changes
      try {
        debug('\tgetDiffFromTarget: calling targetModel.changes with since: %s', sinceSafe)
        await targetModel.changes(sinceSafe, {})
      } catch (err) {
        debug('\tgetDiffFromTarget: error calling targetModel.changes: %s', err.message)
      }
      
      // If there are no source changes, return empty result after calling changes
