...
if (modelName.length === 18 && modelName > 'AnonymousModel_395') {
  console.log('Detaching model start', modelName)
}
  ...
  // Clean any potential references in Loopback's current context (if using LoopBack context)
  try {
    const LoopBackContext = require('loopback-context');
    const currentContext = LoopBackContext.getCurrentContext();
    if (currentContext && currentContext.get && currentContext.set) {
      // Try to remove any reference to this model from current context
      const contextModels = currentContext.get('models');
      if (contextModels && contextModels[modelName]) {
        delete contextModels[modelName];
        currentContext.set('models', contextModels);
        debug('Cleaned up model reference from current LoopBack context');
      }
    }
  } catch (e) {
    // Ignore errors if loopback-context not available
    debug('Could not access LoopBack context: %s', e.message);
  }
