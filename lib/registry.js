...
// Global lock for model detachment operations
const detachLocks = {};
...
Registry.prototype.detachModel = function(modelName, app) {
  // Create a lock key specific to this model
  const lockKey = `detach:${modelName}`;

  // Use the AsyncLock mechanism if available, otherwise fallback to direct execution
  if (detachLocks[lockKey]) {
    debug('Detach operation for model %s already in progress, waiting...', modelName);

    // Return a promise that will be resolved when the lock is available
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve(this.detachModel(modelName, app));
      }, 100); // Retry after 100ms
    });
  }

  // Set the lock
  detachLocks[lockKey] = true;

  try {
    return this._doDetachModel(modelName, app);
  } finally {
    // Always release the lock when done
    delete detachLocks[lockKey];
  }
};

Registry.prototype._doDetachModel = function(modelName, app) {
  ...
  // Check if the model has active operations
  if (modelToDetach._activeOperations && modelToDetach._activeOperations > 0) {
    debug('Model %s has %d active operations, waiting...',
          modelName, modelToDetach._activeOperations);

    // Set a flag to prevent new operations
    modelToDetach._detachPending = true;

    // Wait for a short period and retry
    return new Promise(resolve => {
      setTimeout(() => {
        resolve(this._doDetachModel(modelName, app));
      }, 100); // Retry after 100ms
    });
  }

  // Mark as being detached to prevent new operations
  modelToDetach._detachPending = true;
  ...
  // Also remove from definitions if it exists
  // if (this.modelBuilder.definitions && this.modelBuilder.definitions[modelName]) {
  //   delete this.modelBuilder.definitions[modelName];
  // }

  // if (this.modelBuilder.definitions.Business.properties) {
  //   Object.keys(this.modelBuilder.definitions.Business.properties).forEach(propName => {
  //     const prop = this.modelBuilder.definitions.Business.properties[propName]
  //     if (prop.type && (prop.type.name || '').indexOf('AnonymousModel_') === 0) prop.type = null
  //   })
  // }
  ...
  if (modelToDetach.dataSource) {
    ...
    if (ds.connector && ds.connector.dataSource === ds) {
      ...
      // More thorough cleaning of connector caches
      // if (ds.connector._models) {
        // if (ds.connector._models[modelName]) {
        //   delete ds.connector._models[modelName];
        // }

        // Also check for references to this model in other model caches
        // Object.keys(ds.connector._models || {}).forEach(otherModelName => {
        //   const otherModel = ds.connector._models[otherModelName];
        //   if (otherModel && otherModel.relations) {
        //     // Clean relations that might reference our model
        //     Object.keys(otherModel.relations).forEach(relationName => {
        //       const relation = otherModel.relations[relationName];
        //       if (relation && (relation.modelTo === modelToDetach || relation.modelFrom === modelToDetach)) {
        //         relation.modelTo = null;
        //         relation.modelFrom = null;
        //       }
        //     });
        //   }
        // });
  ...
  if (modelToDetach.definition) {
    ...
    // Clean properties and validations that might hold references
    if (definition.properties) {
      Object.keys(definition.properties).forEach(propName => {
        const prop = definition.properties[propName];
        if (prop) {
          // Break the most critical circular reference - prop→model
          if (prop.model) prop.model = null;

          // Clean all references, not just functions
          Object.keys(prop).forEach(key => {
            // Clean functions (already handled)
            if (typeof prop[key] === 'function') {
              prop[key] = null;
            }
            // Clean objects that might indirectly reference the model
            else if (prop[key] && typeof prop[key] === 'object') {
              // Either null out object references or deep clean them
              prop[key] = null;
            }
          });
        }
        ...
        // Clean properties and validations that might hold references
    // if (definition.rawProperties) {
    //   Object.keys(definition.rawProperties).forEach(propName => {
    //     const prop = definition.rawProperties[propName];
    //     // Clear any function references in the property definition
    //     if (prop) {
    //       Object.keys(prop).forEach(key => {
    //         if (typeof prop[key] === 'function') {
    //           prop[key] = null;
    ...
    //   definition.rawProperties.__proto__ = null;
    // }

    // definition.properties.__proto__

    // Clean up any other references in definition
    ['relations', 'settings', 'properties'].forEach(key => { // rawProperties
