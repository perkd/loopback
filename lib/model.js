...
module.exports = function(registry) {
   ...
   * @return {Promise} If no callback is provided, returns a Promise.
   */
  Model.checkAccess = function(token, modelId, sharedMethod, ctx, callback) {
    // Handle optional ctx parameter
    ...
    if (!callback) {
      return aclModel.checkAccessForContext(context)
        .then(accessRequest => accessRequest.isAllowed())
    }

    // Use async/await to handle the Promise-based checkAccessForContext
    aclModel.checkAccessForContext(context)
      .then(accessRequest => {
   ...
   * 
   * @callback {Function} callback Optional callback function called with `(err, app)` arguments.
   ...
   * @return {Promise} If no callback is provided, returns a Promise.
   */
  Model.getApp = function(callback) {  // FIXME: promise?
    if (!callback) {
      return new Promise((resolve, reject) => {
        this._runWhenAttachedToApp(app => {
          assert(this.app)
          assert.equal(app, this.app)
          resolve(app)
        })
      })
    }
  ...
  function convertNullToNotFoundError(ctx, cb) {
    ...
    const msg = fk ?
      g.f('Unknown "%s" id "%s".', modelName, fk) :
      g.f('No "%s" instance(s) found', modelName)
