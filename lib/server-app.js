...
proto._sanitizePathsForExpress = function(paths) {
  if (Array.isArray(paths)) return paths.map(sanitizePath);
  return sanitizePath(paths);
};

// Convert a single string path that would trip path-to-regexp (Express v5) into
// a RegExp. The path is encoded with encodeURI first so the regex matches what
// arrives in req.url — clients percent-encode characters like ^ before sending.
function sanitizePath(path) {
  if (typeof path !== 'string') return path;

  const hasWildcard = path.includes('*') && !path.includes(':');
  const hasSpecial = /[.+?^${}()|[\]\\!]/.test(path);
  if (!hasWildcard && !hasSpecial) return path;

  const encoded = encodeURI(path);
  let pattern = encoded.replace(/[.+?^${}()|[\]\\!]/g, '\\$&');
  if (hasWildcard) pattern = pattern.replace(/\*/g, '.*');
