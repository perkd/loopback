...
proto.middlewareFromConfig = function(factory, config) {
  ...
  let sanitizedPaths = config.paths;
  if (Array.isArray(config.paths)) {
    sanitizedPaths = config.paths.map(path => {
      if (typeof path === 'string') {
        // Handle special regex patterns that cause path-to-regexp parsing errors
        // Convert patterns like "$!^(explorer)" to proper regex patterns
        if (path.match(/[\$\!\^\(\)]/)) {
        ...
        // Convert wildcard patterns to proper Express patterns
        // Handle patterns like "/api/_m-*" which cause issues in Express v5
        if (path.includes('*') && !path.includes(':')) {
          // Convert "/api/_m-*" to "/api/_m-*" as a regex pattern
