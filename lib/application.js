...
app.model = function(Model, config) {
  ...
  // Models attached to a datasource are already registered by the juggler's
  // attach flow. Register only app-only models here to avoid duplicating the
  // same constructor in multiple owner buckets.
  try {
    const { ModelRegistry } = require('loopback-datasource-juggler');
    if (typeof ModelRegistry.registerModel === 'function' && !Model.dataSource) {
      ModelRegistry.registerModel(Model, Model.definition && Model.definition.properties);
    }
  } catch (err) {
    // loopback-datasource-juggler not available or too old - ignore
  }

  Model.emit('attached', this);

  // Auto-register junction models created by hasAndBelongsToMany relationships.
  // Avoid queueing no-op tasks for the common case where the model has no relations.
  if (Model.dataSource && hasDeclaredRelations(Model)) {
    const self = this;
    // Use setImmediate to ensure the relationship setup is complete
    setImmediate(function() {
      self._autoRegisterJunctionModels(Model);
    });
  }

  refreshSharedMethodConfiguration(this);
 ...
 * @private
 */
app._autoRegisterJunctionModels = function(Model) {
  if (!Model.dataSource || !Model.relations) return;

  const self = this;
  const dataSource = Model.dataSource;

  // Check for hasMany relationships that might be part of hasAndBelongsToMany
  for (const relationName in Model.relations) {
    const relation = Model.relations[relationName];

    // Check all datasource models to see if any look like junction models
    for (const dsModelName in dataSource.models) {
      const dsModel = dataSource.models[dsModelName];
      // Check if this looks like a junction model for this relationship
      if (dsModelName.includes(Model.modelName) &&
          relation.modelTo &&
          dsModelName.includes(relation.modelTo.modelName) &&
          !self.models[dsModelName]) {
        // Register the junction model with the app, ensuring it has the dataSource
        self.model(dsModel, {dataSource: dataSource, public: false});
      }
    }
  }
};

function hasDeclaredRelations(Model) {
  return !!(Model && Model.relations && Object.keys(Model.relations).length);
}

function refreshSharedMethodConfiguration(app) {
  const remotingConfig = app.get('remoting');
  const hasAppConfig = remotingConfig &&
  ...
  app.models().forEach(function(Model) {
    ...
    const hasModelConfig = options &&
      options.remoting &&
      options.remoting.sharedMethods &&
      typeof options.remoting.sharedMethods === 'object';

    if (!hasAppConfig && !hasModelConfig) return;
    configureSharedMethods(Model, remotingConfig, Model.config);
  });
}
...
app.enableAuth = function(options) {
  ...
  if (options && options.dataSource) {
    ...
    AUTH_MODELS.forEach(function(m) {
      ...
      let hasAttachedSubclass = false;

      if (useOwnerAwareQueries) {
        // Use efficient owner-aware queries from centralized model registry
        ...
        // Note: We don't add deprecation warning here since this is internal LoopBack code
        // and the warning would be confusing to end users
        ...
        for (const name in appModels) {
          ...
          if (isSubclass && isAttached) {
            hasAttachedSubclass = true;
            break;
          }
        }
      }

      if (hasAttachedSubclass) return;
  ...
  remotes.authorization = function(ctx, next) {
    ...
    if (Model.checkAccess) {
      Model.checkAccess(req.accessToken, modelId, method, ctx)
        .then((allowed) => {
          if (allowed) {
            next()
          }
