...
function AccessContext(context) {
  ...
  if (!this.principals.length && context.principalType && context.principalId) {
    const BaseUser = this.registry.getModel('User');
    let principalType = context.principalType;
    if (!this.registry.findModel(principalType)) {
      principalType = BaseUser && BaseUser.modelName || context.principalType;
    }
    this.principals.push(new Principal(principalType, context.principalId));
  }
 ...
 * @returns {*}
 */
AccessContext.prototype.getUser = function(cb) {
  const BaseUser = this.registry.getModel('User');
  let result;
  for (let i = 0; i < this.principals.length; i++) {
    const p = this.principals[i];
    ...
    if (p.type === Principal.USER) {
      result = {id: p.id, principalType: p.type};
      break;
    }
